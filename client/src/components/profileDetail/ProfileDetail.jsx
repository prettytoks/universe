...
const ProfileDetail = () => {
  ...
  useEffect(() => {
    const fetchProfile = async() => {
       try {
        const res = await fetch(`${process.env.REACT_APP_SERVER_URL}/user/find/${id}`, {
          headers: {
  ...
  useEffect(() => {
    const fetchProfilePosts = async() => {
      try {
        const res = await fetch(`${process.env.REACT_APP_SERVER_URL}/post/find/userposts/${id}`)
  ...
  const handleFollowFunction = async() => {
    try {
      await fetch(`${process.env.REACT_APP_SERVER_URL}/user/toggleFollow/${profile?._id}`, {
        headers: {
          'Authorization': `Bearer ${token}`
        },
        method: "PUT"
